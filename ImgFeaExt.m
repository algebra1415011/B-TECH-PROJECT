...
%%step2: Check totalPixelsOfImage
...
%%step3: split image into h, s & v planes
...
%%step4: Quantize each h,s,v equivalently to 8x2x2
...
hsvColorHistogram(index(row, 1), index(row, 2), index(row, 3)) = hsvColorHistogram(index(row, 1), index(row, 2), index(row, 3)) + 1;
...
clear('row', 'col', 'count', 'numberOfLevelsForH', 'numberOfLevelsForS','numberOfLevelsForV', 'maxValueForH', 'maxValueForS', 'maxValueForV','index', 'rows', 'cols', 'h', 's', 'v', 'image', 'quantizedValueForH','quantizedValueForS', 'quantizedValueForV');
