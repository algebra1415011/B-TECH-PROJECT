...
function [RECT, IMR] = facescan(NET, IM, MASK, THR, LEVELS, START, SCALEFACT,STEP)
...
for im_num = START:PYR_MAX,
 ...
 ((im_row+MROWS-1)/size(YRANGE{im_num},2))*size(YRANGE{1},2), ...((im_col+MCOLS-1)/size(XRANGE{im_num},2))*size(XRANGE{1},2), ...
 TEST];
 end
 end
 end
